...
const LiveSetDataSchema = z.object({
  ...
  value: z.string().describe("The current total trading value in millions of Baht, e.g., '25,433.34'."),
...
const getSetDataFlow = ai.defineFlow(
  ...
  async ({type, days}) => {
    ...
    if (type === 'live') {
        const prompt = `Go to https://www.set.or.th/en/market/index/overview and find the current SET Index value, its total trading value (Value), and its update time. The value is usually a large number with two decimal places. For example, "1,345.67". The time is usually displayed next to it. Provide the most recent value available.`;
